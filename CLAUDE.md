...
## Modüler Yapı
- Çekirdek `app/` altındadır ve hep açıktır: giriş/kayıt/parola, hesap etkinleştirme (hesabı olmayan kişi — ör. içe aktarılan üye — `/activate`'te e-postasını yazar veya yönetici kişi sayfasından bağlantı gönderir; imzalı, 48 saatlik, e-posta değişince geçersizleşen bağlantıyla parola belirler; hesap mevcut kişiye bağlanır, sıfat ve üyelik korunur; `App\Support\AccountActivation`), telefon doğrulama (`phone_verifications`, eski adı `contact_permissions`), etiketler (`tags`, `contact_tag`; yönetim `/admin/tags`, kişi listesinde filtre) ve özel alanlar (`custom_fields`, `custom_field_values`; `/admin/custom-fields`, `fields.manage`; tür, grup, zorunluluk, kişinin erişimi hidden/visible/editable; kişi sayfasında ve profilde düzenlenir, kimlik kartına `custom.<anahtar>` ile basılabilir; gruplar `App\Support\CustomFields::group()` ile, modül `$this->customFields()->group(...)`; derneğe özgü alanlar sabit sütun yerine buradan tanımlanır), iletişim izinleri (`consent_events`: kanal başına son olay geçerli izindir; `App\Support\Consents::set()` yalnız değişeni kaydeder, gönderimden önce `allows($contact, 'email'|'sms'|'whatsapp'|'phone')`; profil sayfası, kayıt formu ve yönetimde kişi sayfası), profil, profil fotoğrafı (`contact_photos`: kişi profil sayfasından (`/my-infos`, `#photo`) yükler, `photos.review` yetkilisi `/admin/photos`'ta onaylar; dosyalar gizli `local` diskte, yalnız `photos.show` route'uyla yetkiliye sunulur), kullanıcı/rol yönetimi, TC doğrulama, işlem kayıtları (`process_logs`; `App\Models\Concerns\Auditable` takılı modeller oluşturma/değişiklik/silmeyi eski-yeni değerle kendiliğinden kaydeder, kimlik no maskelenir, parola kaydedilmez; model olayı olmayan değişiklik için `App\Support\Audit::record()`; yönetim `/admin/process-logs`), sözleşmeler (`agreements` / `agreement_versions` / `agreement_acceptances`: yönetim `/admin/agreements`, `agreements.manage`; taslak → yayınla ile yeni sürüm, yayınlanan sürüm değişmez; formlarda `<x-agreement-checkbox key="kvkk" />`, doğrulama `Agreements::rules('kvkk')`, kayıt `Agreements::accept($user, '<bağlam>', 'kvkk')`; yayında sürüm yoksa kutu gösterilmez; herkese açık metin `/agreements/{anahtar}`), KVKK veri silme talebi (`data_deletion_requests`: kişi profilden parolasıyla ister, `data-deletion.manage` yetkilisi `/admin/data-deletions`'ta onaylar/reddeder; onay `App\Support\Anonymizer` ile kişi ve hesabı anonimleştirir, satırları silmez; eski denetim kayıtlarındaki değerleri de temizler; yönetimde onay öncesi uyarılar için `admin.data-deletion.notes` slotu), kurum ayarları (`settings` tablosu, `App\Support\Organization`; view'larda `$organization`: ad, kısa ad, logo, favicon, ana renk, iletişim, sosyal bağlantılar, bildirim adresi, duyuru göndericisi, iframe `frame-ancestors` siteleri, Google Analytics/Tag Manager kimlikleri, editörle düzenlenen ana sayfa; yönetim: `/admin/settings/organization`, `settings.manage` yetkisi), mesajlaşma kanalları (`App\Contracts\Messaging\SmsSender` / `WhatsAppSender`; sürücüler `config/messaging.php`: NetGSM, WhatsApp bridge veya `log`; bildirimler `App\Notifications\Channels\SmsChannel`/`WhatsAppChannel` ile `toSms()`/`toWhatsApp()`; bilgilendirme mesajı izne bağlı olarak `App\Support\Messaging\Messenger::inform()` ile), ortak servisler (`HtmlSanitizer`, `MailgunMailingList`, `AnnouncementMailing`).
...
- Kuyruk: `QUEUE_CONNECTION=database` (`jobs` tablosu); e-postalar (`ShouldQueue` mailable'lar) worker'da gönderilir. Worker systemd servisi `deploy/portal-queue.service` (www-data, `queue:work database`); her deploy sonrası `sudo -u www-data php8.4 artisan queue:restart`. Başarısız işler: `php8.4 artisan queue:failed` / `queue:retry all`. `ForwardingWelcome` bilerek senkron (modele sonradan eklenen `alias` alanını taşır).
